...
my $test2 = start { # Test paired files
    ...
    indir( $paired_test_dir, {
        ...
        my $raw_job_id = qx{ ../../bin/sbatch_script --sarray_file_pattern='*_R1_001.fastq' --sarray_paired_file_pattern='*_R2_001.fastq' get_seqs 'awk "\{if (FNR % 4 == 2) print\}" $FILE > forward.$SLURM_ARRAY_TASK_ID.seqs; awk "\{if (FNR % 4 == 2) print\}" $PAIRED_FILE > reverse.$SLURM_ARRAY_TASK_ID.seqs; '};
        my $job_id     = get_jobid($raw_job_id);
        my $wait_step  = qqx{ sbatch --output='wait.o_%j' --wait --dependency=$job_id --wrap='echo "Finished all jobs: ($job_id)"'};
...
my $test4 = start { # Test prefix extraction
    ...
    indir( $prefix_test_dir, {
        ...
        my $raw_job_id = qx{ ../../bin/sbatch_script --sarray_file_pattern='*_R1_001.fastq' --sarray_paired_file_pattern='*_R2_001.fastq' check_prefixes 'echo "$FILENAME_PREFIX" > temp_prefix_$FILENAME_PREFIX.txt'};
        my $job_id     = get_jobid($raw_job_id);
        my $wait_step  = qqx{ sbatch --output='wait.o_%j' --wait --dependency=$job_id --wrap='echo "Finished all jobs: ($job_id)"'};
